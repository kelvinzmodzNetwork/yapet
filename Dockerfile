...
# Ekspos port yang digunakan NoVNC (80 di dalam container)
EXPOSE 80
...
# Buat skrip untuk menjalankan ngrok dan NoVNC
RUN echo "/ngrok http 80 --authtoken ${NGROK_TOKEN} &" >> /docker.sh \
    && echo "sleep 5" >> /docker.sh \
    && echo "curl -s http://localhost:4040/api/tunnels | python3 -c \"import sys, json; print(\\\"Access NoVNC via:\\\", json.load(sys.stdin)['tunnels'][0]['public_url'])\" || echo \"Error: NGROK_TOKEN, reset token ngrok dan coba lagi\"" >> /docker.sh \
    && echo 'supervisord -c /etc/supervisor/supervisord.conf' >> /docker.sh \
    && chmod 755 /docker.sh

# Jalankan ngrok dan NoVNC dengan Supervisor
